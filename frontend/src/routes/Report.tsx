...
const HEADLINE = [
  ...
  { label: "튜닝 하이브리드 (nDCG@10)", value: "0.88→0.95", tag: "FT dense 위 RRF 융합", sub: "은어 0.77→0.92" },
  { label: "정답 역전 사례 (은어)", value: "36건", tag: "base 오답→FT 정답", sub: "23개 시스템" },
  { label: "자동화 테스트", value: "208건", tag: "전 구간 회귀 방어", sub: "" },
] as const;
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8 pb-20">
      ...
      <Topic id="bench" n="03" title="관련연구 벤치마킹 조사">
        ...
        <div className="pt-1 text-[12.5px] font-semibold text-mut">공개 모델 서베이 — 그대로 쓸 수 있는 게 있나 (dev, zero-shot)</div>
        <Tbl
          head={["옵션 (7종 zero-shot)", "계열", "nDCG@10", "은어 nDCG"]}
          rows={[
            ["BM25 (문자 bigram)", "어휘", "0.569", "0.249"],
            ["KURE-v1 (한국어 특화)", "dense", "0.561", "0.193"],
            ["Qwen3-0.6B", "dense", "0.559", "0.152"],
            ["multilingual-e5-large", "dense·다국어", "0.552", "0.182"],
            ["bge-m3", "dense·다국어", "0.544", "0.159"],
            ["Nemotron-1B", "dense", "0.543", "0.171"],
            ["Qwen3-0.6B — 파인튜닝", "dense", "0.879", "0.772"],
          ]}
          emph={[6]}
        />
        <Note tone="signal" title="결론 — 한국어 특화 모델조차 우리 은어를 모른다">
          공개 옵션 7종(dense 6·어휘 1)이 계열 불문 <M>nDCG@10 0.54~0.57</M>에 몰려 있고 은어는
          0.15~0.25로 붕괴합니다. 결정적으로 <b className="text-fg">한국어 특화 KURE-v1조차 은어 0.193</b>으로
          참패해요(공개 중 최고지만 파인튜닝 0.772의 1/4). 즉 <b className="text-fg">"한국어를 아느냐"가
          아니라 "우리 조직 은어를 아느냐"의 문제이고, 답은 계열·한국어 특화 여부와 무관하게 모른다</b>.
          이 지형을 벗어나는 유일한 수단이 <b className="text-fg">파인튜닝</b>(0.879)이라, 차기 모델 결정은
          "어떤 공개 모델을 쓸까"가 아니라 <b className="text-fg">"어떤 base를 파인튜닝할까"</b>가 됩니다.
        </Note>
      </Topic>
      ...
      <Topic id="journey" n="04" title="과제추진 사항 — 경과와 선택 근거">
        ...
        <Panel className="p-5">
          ...
          <TimelineNode
            date="2026-07-24"
            phase="하이브리드·정성"
            title="RRF 융합 2×2 + 파인튜닝 전후 정성 분석"
            did={<>dense와 BM25를 <M>RRF</M>로 실제 융합해 dev에서 가중 α를 튜닝(<M>rag/evaluation/hybrid.py</M>). base·FT × dense·하이브리드 2×2를 재고, 파인튜닝 전후 랭킹을 쿼리 단위로 비교해 정답 역전 사례를 추출.</>}
            why={<>3.4의 상보성은 "천장이 오른다"는 <b className="text-fg">진단</b>이지 시스템이 아님 — 실제로 융합해 이겨야 보조 목표가 성과가 됨. 그리고 집계 nDCG는 "무엇이" 좋아졌는지 말하지 않으므로 눈으로 볼 사례가 필요.</>}
            result={<>FT 위 융합 <M>0.879→0.954</M>(p=1e-4). 단, <b className="text-fg">하이브리드는 FT를 대체 못 함</b>(base 융합은 은어 0.15→0.20뿐) — FT가 선행 조건. 정성: 은어 <b className="text-fg">36건</b>이 오답 시스템→정답 시스템으로 역전.</>}
          />
          <TimelineNode
            date="계획"
            phase="다음"
      ...
      <Topic id="value" n="05" title="과제성과/기대효과">
        ...
        <div className="pt-1 text-[12.5px] font-semibold text-mut">하이브리드 융합 — dense가 BM25 위에 얹히나 (dev 168쿼리, nDCG@10 / 은어)</div>
        <Tbl
          head={["", "dense 단독", "튜닝 하이브리드 (RRF)", "dense 대비"]}
          rows={[
            ["base (zero-shot)", "0.559 (은어 0.152)", "0.587 (은어 0.201)", "+0.028 (p=0.012)"],
            ["파인튜닝", "0.879 (은어 0.772)", "0.954 (은어 0.918)", "+0.075 (p=1e-4, 28승2패)"],
          ]}
          emph={[1]}
        />
        <Note tone="signal" title="핵심 — 하이브리드는 파인튜닝을 대체하지 못한다">
          base에 BM25를 융합해도 은어는 <M>0.152→0.201</M>로 거의 안 움직입니다 — 은어는 문서에 없어 BM25도
          못 풀고, base dense도 모르니 무지한 두 신호를 섞어도 무지합니다. 큰 융합 이득(<M>0.879→0.954</M>)은
          <b className="text-fg"> 파인튜닝이 은어→시스템을 가르친 뒤에야</b> 나옵니다: 그때 BM25의 과업 어휘
          ("지출결의"·"OTP 등록")가 순위를 다듬죠. <b className="text-fg">FT가 선행 조건, 하이브리드는 그 위에.</b>
          {" "}단 이 dev 이득은 dense가 이미 천장인 final(0.983)에선 재현 안 됨 — 하이브리드는 dense에 여지가
          있을 때만 유효한, 배포처마다 튜닝하는 부품입니다.
        </Note>

        <div className="pt-1 text-[12.5px] font-semibold text-mut">정성 분석 — 모델이 배운 것 (은어 정답 역전, 36건 중)</div>
        <Tbl
          head={["은어 쿼리", "base 1위 (오답)", "파인튜닝 1위 (정답)"]}
          rows={[
            ["파피루스 지출결의 내는 법", "피플온 사용 가이드", "한결 사용 가이드"],
            ["개미굴 OTP 등록 어디서 해", "허브포트 사용 가이드", "게이트원 사용 가이드"],
            ["우물 SQL 돌릴 곳 신청", "타운홀 사용 가이드", "데이터온 사용 가이드"],
            ["자작나무 야근 등록 어디서 해", "워크로그 릴리스 노트", "워크로그 사용 가이드"],
            ["감나무 요즘 어디서 봐", "메일가드 장애 공지", "노트리 (사내 위키)"],
          ]}
        />
        <Note tone="cyan" title="두 가지 실패가 고쳐진다">
          <b className="text-fg">틀린 시스템</b>(파피루스→피플온이 아니라 한결): base는 은어를 몰라 그럴듯한 다른
          시스템을 집고, FT는 은어→시스템 매핑을 배워 정답으로 갑니다. <b className="text-fg">맞는 시스템·틀린
          문서</b>(자작나무→릴리스 노트가 아니라 사용 가이드): 사용자의 실제 의도(방법 문서)를 FT가 더 정확히
          맞춥니다.
        </Note>
      </Topic>
      ...
      <Topic id="future" n="06" title="향후계획 / 연구보완">
        <Tbl
          ...
          rows={[
            ...
            ["3", "서베이·튜닝 스윕 실데이터로", "공개 모델 7종 서베이는 인트라넷 셋에서 완료(§3) — 실 corpus에서 재확인 + 각 base를 제대로 튜닝(좌표하강 스윕)해 파인튜닝 후 순위까지 확정. 설계서·하니스 완비, CUDA 서버 실행."],
            ["4", "서빙 벤치 CUDA 재측정", "지연·GPU 절대 수치 확보. 층수↔폭의 속도 관계가 GPU 커널 특성상 뒤집힐 수 있어 필요."],
            ["5", "하이브리드 실데이터 확정", "대역 실험에서 'FT가 은어 열고→융합이 순위 정렬' 방향 확인(dev +0.075). 프로덕션 형태소 BM25+리랭커 위에서 α 재튜닝·확정."],
