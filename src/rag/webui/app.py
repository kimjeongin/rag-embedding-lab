...
(gradio-free); this file is layout + copy + wiring. The layout is a small design
system: a hero header, then card-wrapped sections per tab. Each tab surfaces the
concrete data it consumes (file + counts) so the data → train → eval → compare flow is
visible, not just implied by tab order. Radio/Dropdown use (label, value) pairs so the
display text is decoupled from the stable value handlers switch on.
"""
...
def build_ui() -> gr.Blocks:
    with gr.Blocks(title="RAG Embedding Lab", fill_width=True) as demo:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("① 데이터", id="tab_data"):
                ...
                with gr.Group(elem_classes="card"):
                    ...
                    with gr.Accordion("📋 전체 데이터 보기", open=False):
                        d_pairs_full = gr.Dataframe(actions.full_pairs, interactive=False)
                ...
                with gr.Group(elem_classes="card"):
                    ...
                    with gr.Accordion("📋 전체 데이터 보기", open=False):
                        d_eval_full = gr.Dataframe(actions.full_corpus, interactive=False)
        ...
        ).then(actions.full_pairs, None, d_pairs_full).then(
        ...
        d_eval_btn.click(actions.gen_eval_set, [d_ndist], [d_eval_status, d_eval_prev]).then(
            actions.full_corpus, None, d_eval_full
