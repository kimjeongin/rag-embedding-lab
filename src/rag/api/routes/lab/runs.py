...
from rag.diff import DEFAULT_METRIC, compare_runs
...
from rag.evaluation.metrics import bootstrap_ci, mean_metrics, per_query_metrics
...
def diff_runs(a: str, b: str, metric: str = DEFAULT_METRIC) -> DiffResponse:
    ...
    try:
        result = compare_runs(run_a, run_b, metric)
    except ValueError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc

    # Join query texts + retrieved doc titles — only valid while the eval set on disk
    # still has the exact contents these runs were measured on.
